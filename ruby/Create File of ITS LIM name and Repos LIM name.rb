 ...
 its_lims_in = File.new("C:/Users/rod_brindamour/Documents/Clients/ITS/Repository/All LIMs for Review.txt","r")
 ...
 its_lims_array.each {|its_uc_number,its_uc_name,its_category,its_screen,its_lim|
      if its_uc_number == "03.01.022"
       puts "#{its_uc_number}\t#{its_screen}\t#{its_lim}\t#{its_lim.match(/[_|\.\/]((\w*.|)\w*$)|(\w*.designer.cs)/)[0].gsub(/\//,"")}"
      end
       if its_lim
               temp_program_name = its_lim.match(/[_|\.\/]((\w*.|)\w*$)|(\w*.designer.cs)/)
               if temp_program_name then
                program_name = temp_program_name[0].gsub(/\//,"")
               else
                program_name = " "
               end
       end
      ...
      #puts its_lim
       ...
       if its_lim
         #puts "its_lim = #{its_lim} program name = #{program_name}"
           if (lim_string.include? its_lim.downcase) | (lim_string.include? program_name) then
               lim_array.each {|lim,pgm| 
                                   if lim.match(its_lim.downcase) then
                                        (lim_name = lim)
                                        next
                                   end
                              }
               ...
               if lim_string.include? program_name
                    lim_array.each {|lim,pgm|
